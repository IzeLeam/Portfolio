  ...
  try {
    ...
    if (PRIORITY_ORDER.length) {
      const indexMap = new Map(
        PRIORITY_ORDER.map((name, i) => [name.toLowerCase(), i] as const)
      );
      results.sort((a, b) => {
        const ai = indexMap.has(a.name.toLowerCase())
          ? indexMap.get(a.name.toLowerCase())!
          : Infinity;
        const bi = indexMap.has(b.name.toLowerCase())
          ? indexMap.get(b.name.toLowerCase())!
          : Infinity;
